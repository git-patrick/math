...
#ifndef math__Derivative_h
#define math__Derivative_h
...
namespace math {
	...
	namespace detail {
		template <typename R, typename I, std::size_t A>
		struct __D<complex<R,I>, select<A>, derivative_analytic_stage> {
			template <typename __T=R> using type = rational<0>;
		};
		template <std::intmax_t N, std::intmax_t D, std::size_t A>
		struct __D<rational<N,D>, select<A>, derivative_analytic_stage> {
		...
		template <typename F, typename G, typename dx>
		struct __D<__pow<F,G>, dx, derivative_analytic_stage> {
